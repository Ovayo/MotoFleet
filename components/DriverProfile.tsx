...
const DriverProfile: React.FC<DriverProfileProps> = ({ 
  ...
  const stats = useMemo(() => {
    // Current logical week for simulation
    const CURRENT_WEEK = 4;
    const driverPayments = (payments || []).filter(p => p?.driverId === driver.id);
    
    // Weekly Standing Logic (Less Intimidating)
    ...
    for (let p of sortedPayments) {
      if (p && p.amount >= (driver.weeklyTarget || weeklyTarget)) streak++;
      else break;
    }
    ...
    if (routineServices.length > 0) {
      ...
      serviceDaysDiff = isNaN(latestTime) ? 999 : Math.floor((new Date().getTime() - latestTime) / (1000 * 60 * 60 * 24));
    }
  ...
  }, [payments, fines, driver.id, driver.weeklyTarget, weeklyTarget, bike, maintenance]);
            ...
            <div className="flex items-center space-x-8">
               ...
               <div className="space-y-2">
                <div className="inline-flex items-center bg-white/20 backdrop-blur-md px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-white/20">
                  ...
                  {stats.weeklyStanding >= 0 ? 'Verified Weekly Standing' : 'Active Settlement Cycle'}
                </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col justify-between relative overflow-hidden">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Weekly Standing (Week {stats.currentWeek})</p>
                <div className="flex flex-col">
                  <div className="flex items-baseline space-x-2">
                    <h3 className={`text-4xl font-black ${stats.weeklyStanding >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                      R{Math.abs(stats.weeklyStanding)}
                    </h3>
                    <span className="text-gray-400 text-xs font-black uppercase">{stats.weeklyStanding >= 0 ? 'Surplus' : 'To Pay'}</span>
                  </div>
                  {stats.totalUnpaidFines > 0 && (
                    <div className="mt-2 text-red-400 text-[10px] font-bold uppercase tracking-widest bg-red-50 px-3 py-1 rounded-full w-fit">
                      {stats.totalUnpaidFines} in Fines
                    </div>
                  )}
                </div>
                <div className="mt-6 pt-6 border-t border-gray-50 flex flex-col space-y-2">
                   <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                     ...
                     <span className="text-gray-800">R{stats.targetThisWeek}</span>
                   </div>
                   <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                     <span className="text-gray-400">Paid this week:</span>
                     <span className="text-green-600">R{stats.paidThisWeek}</span>
                   </div>
                </div>
              </div>
              ...
              <div className="bg-gray-900 p-8 rounded-[2.5rem] shadow-xl text-white flex flex-col justify-center">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Traffic Notices</p>
                <div className="flex items-center justify-between">
                   <div>
                     <p className="text-4xl font-black text-red-500">{stats.driverFines.length}</p>
                     <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">Notices</p>
                   </div>
                   <div className="text-right">
                     <p className="text-2xl font-black text-white">R{stats.totalUnpaidFines}</p>
                     <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest mt-1">Outstanding</p>
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white rounded-[3rem] shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-8 border-b border-gray-50 flex justify-between items-center">
                   <h3 className="font-black text-gray-800 uppercase text-xs tracking-[0.2em]">Recent Collections</h3>
                  ...
                  {(payments || [])
                    ...
                    .map(p => (
                      <div key={p.id} className="p-6 flex justify-between items-center hover:bg-gray-50/50 transition-colors">
                        ...
                        <p className={`text-lg font-black ${p.amount >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                          {p.amount >= 0 ? `+R${p.amount}` : `R${p.amount}`}
                        </p>
                      </div>
                    ))}
                  {payments.filter(p => p.driverId === driver.id).length === 0 && (
                    <div className="py-20 text-center text-gray-300 font-black uppercase text-[10px] tracking-widest">No data</div>
                  )}
                </div>
              </div>
