  ...
  return (
    <div className="space-y-10">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total Operator Payments" value={`R${totalPaidByOperators.toLocaleString()}`} icon="💰" color="blue" subtitle="Lifetime Gross" />
        <StatCard title="Net Fleet Yield" value={`R${netProfit.toLocaleString()}`} icon="📊" color="green" subtitle="Post-Maintenance" />
        <StatCard title="Grid Strength" value={`${bikes.length} UNITS`} icon="🏍️" color="indigo" subtitle="Asset Count" />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="bg-white/80 dark:bg-gray-900/60 backdrop-blur-3xl p-10 rounded-[3.5rem] shadow-sm border border-white/60 dark:border-white/5">
               <div className="flex justify-between items-center mb-8">
                  <h3 className="text-[10px] font-black text-gray-900 dark:text-white uppercase tracking-[0.3em]">Critical Arrears</h3>
                   ...
                   <div key={driver.id} className="flex items-center justify-between p-5 bg-white/50 dark:bg-white/5 rounded-[2rem] border border-white/60 dark:border-white/5 hover:shadow-xl transition-all duration-500 group">
                      <div className="flex items-center space-x-4 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[9px] text-gray-400 dark:text-white/30 font-bold uppercase tracking-widest mt-1.5">Hub: {driver.city}</p>
                        </div>
                      </div>
            ...
            <div className="bg-gray-900 p-10 rounded-[3.5rem] shadow-2xl text-white relative overflow-hidden group border border-white/5">
               ...
               <div className="space-y-6">
                  ...
                  <button className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl text-[9px] font-black uppercase tracking-[0.3em] transition-all text-white/80">Analyze Force Multipliers</button>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-10">
          ...
          <div className="bg-white/70 dark:bg-gray-900/60 backdrop-blur-3xl p-10 rounded-[3.5rem] border border-white/60 dark:border-white/5 shadow-sm">
             <h4 className="text-[10px] font-black text-gray-400 dark:text-white/30 uppercase tracking-[0.3em] mb-8">System Compliance</h4>
             <div className="space-y-6">
                <div className="flex items-center justify-between">
                   <span className="text-[11px] font-black text-gray-800 dark:text-white uppercase tracking-tight">eNaTIS Link</span>
                   <span className="text-[9px] font-black text-green-500 dark:text-green-400 bg-green-50 dark:bg-green-400/10 px-2 py-1 rounded-full uppercase">Verified</span>
                </div>
                <div className="flex items-center justify-between">
                   <span className="text-[11px] font-black text-gray-800 dark:text-white uppercase tracking-tight">Banking Ledger</span>
                   <span className="text-[9px] font-black text-blue-500 dark:text-blue-400 bg-blue-50 dark:bg-blue-400/10 px-2 py-1 rounded-full uppercase">Sync Active</span>
                </div>
                <div className="flex items-center justify-between">
                   <span className="text-[11px] font-black text-gray-800 dark:text-white uppercase tracking-tight">Telematics Grid</span>
                   <span className="text-[9px] font-black text-amber-500 dark:text-amber-400 bg-amber-50 dark:bg-amber-400/10 px-2 py-1 rounded-full uppercase">100% Signal</span>
                </div>
             </div>
          </div>
        </div>
